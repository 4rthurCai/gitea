...
func Activity(ctx *context.Context) {
	...
	if err != nil {
		ctx.ServerError("IsRepositoryExist(wiki)", err)
		return
	}
	if hasWiki {
		...
		if ctx.PageData["repoWikiActivityTopAuthors"], err = activities_model.GetActivityStatsTopAuthorsByStorageRepo(ctx, ctx.Repo.Repository.WikiStorageRepo(), timeFrom, 10); err != nil {
