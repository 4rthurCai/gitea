...
import RepoWikiActivityTopAuthors from '../components/RepoWikiActivityTopAuthors.vue';
...
export function initRepoWikiActivityTopAuthorsChart() {
  const el = document.querySelector('#repo-wiki-activity-top-authors-chart');
  if (el) {
    createApp(RepoWikiActivityTopAuthors).mount(el);
  }
}
